...
class RNN(nn.Module):
    def __init__(self, config, dataset):
        ...
        self.in_tok_embed = nn.Embedding(dataset_size, self.embedding_size, self.padding_id)
        ...
        self.out_mlp = nn.Linear(self.hidden_dim, self.embedding_size)
        self.encoder = nn.LSTM(self.embedding_size, self.hidden_dim, batch_first=True)
        self.decoder = nn.LSTM(self.embedding_size, self.hidden_dim, batch_first=True)
    ...
    def forward(self, batch, dataloader, is_display):
        ...
        mask_matrix = in_mask_matrix.to(torch.bool)#.contiguous()# .cuda(self.device)
        ...
        # decode target data
        target_seq_rep, _ = self.decoder(target_seq_rep, (h_t, c_0))
        target_seq_rep = F.dropout(target_seq_rep, p=self.dropout)
        probs = torch.softmax(self.out_tok_embed(self.out_mlp(target_seq_rep)), -1)
        #print(probs[:, :3, :]); exit()
